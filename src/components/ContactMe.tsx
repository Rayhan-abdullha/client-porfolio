                ...
                <form className='md:w-1/2 flex flex-col gap-5 mt-10 md:mt-0'>
                    {
                        contactForm.map(item => (
                            item.type === 'textarea' ? <textarea className={`rounded-xl border p-5 outline-none ${isDarkMode ? 'bg-[#060606] text-[#acacac] border border-[hsla(0,0%,100%,0.1)]' : 'border border-[#aaaaaa] bg-[#f5f8fa]'}`} name='text' placeholder='Enter your message'></textarea> : <input key={item.id} className={`rounded-xl border p-5 outline-none ${isDarkMode ? 'bg-[#060606] text-[#acacac] border border-[hsla(0,0%,100%,0.1)]' : 'border border-[#aaaaaa] bg-[#f5f8fa]'}`} type={item.type} placeholder={item.label} />
                        ))
                    }
