...
const ContactMe = () => {
    ...
    return (
        <div className='container mx-auto px-4 py-12 md:py-[120px]'>
                ...
                <form className='md:w-1/2 flex flex-col gap-5 mt-10 md:mt-0'>
                    {
                        contactForm.map(item => (
                            <input key={item.id} className={`rounded-xl border p-5 outline-none ${isDarkMode ? 'bg-[#060606] text-[#acacac] border border-[hsla(0,0%,100%,0.1)]' : 'border border-[#aaaaaa]'}`} type={item.type} placeholder={item.label} />
                        ))
                    }
